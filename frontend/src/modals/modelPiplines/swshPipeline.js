function _isSwordShieldModel(glbUrl) {
	const u = String(glbUrl || "").toLowerCase();
	return u.includes("/gen8/") && u.includes("/sword-shield/");
}
...
function _swshPieceForMaterial(matName) {
...
async function applySwordShieldTextureSetToScene(root3d, { glbUrl, variant }) {
	// Example glbUrl:
	// .../imgs/sprites/gen8/sword-shield/base-model/130/130.glb
	const baseDir = _dirname(glbUrl);
	...
	async function loadSwshTex(url, { srgb = false } = {}) {
		const t = await _loadTexture(tl, url, { srgb });
	...
	root3d.traverse((o) => {
		...
		for (const oldMat of mats) {
			...
			const piece = _swshPieceForMaterial(matName);
	...
	root3d.traverse((o) => {
		...
		const out = mats.map((oldMat) => {
			const piece = _swshPieceForMaterial(oldMat?.name || "");
